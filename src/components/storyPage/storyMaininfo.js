...
export default function StoryMainInfo({ story, author, userId, chapters }) {
  ...
  const handleReportSubmit = (e) => {
    ...
    if (!storedUserId) {
      alert("Lỗi: Vui lòng đăng nhập để gửi báo cáo.");
      console.log(storedUserId);
      return;
    }

    if (!reportReason.trim() && !reportDetails.trim()) {
      alert("Vui lòng nhập lý do hoặc chi tiết báo cáo.");
      return;
    }
    ...
    const reportData = {
      ...
      status: "in process", // THÊM TRƯỜNG NÀY VÀO ĐÂY
    };
    ...
    <Row>
      ...
      <Col md={9} xs={12}>
        ...
        <div className="d-flex gap-2 flex-wrap mb-3">
          ...
          <Button
            className={`story-button follow-button ${
          ...
          {showReportForm ? renderForm() : null}
        </div>
      </Col>
    </Row>
  );
